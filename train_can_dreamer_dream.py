...
def load_policy(path, rssm, action_dim, device):
    ...
    nets = make_dreamer_dp_policy(rssm.state_dim, action_dim, cfg['pred_horizon'],
...
def dream_value(rssm, obs, actions):
    with torch.no_grad():
        state = raw_condition(rssm, obs, actions, 'rssm')
...
def train(args):
    ...
    with tqdm(range(args.num_epochs), desc='DP-Dreamer dream epoch') as tglobal:
...
def main():
    ...
    parser.add_argument('--output', default='data/outputs/can_dp_dreamer_rssm_dream')
