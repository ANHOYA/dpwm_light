...
from train_rssm_bc import infer_rssm_state


def rssm_condition(rssm, obs, actions, conditioning):
    ...
    if conditioning == 'rssm_imagine':
        ...
        cond = torch.cat([state, imagined.flatten(start_dim=1)], dim=-1)
    return cond
...
def train(args):
    ...
    nets = make_rssm_policy(rssm.state_dim, action_dim, args.pred_horizon,
    ...
    with tqdm(range(args.num_epochs), desc='DP-RSSM epoch') as tglobal:
        for epoch in tglobal:
            ...
            for batch in tqdm(loader, desc='Batch', leave=False):
                ...
                noisy_actions = noise_scheduler.add_noise(target, noise, timesteps)
                cond_actions = target if args.rssm_action_mode == 'clean' else noisy_actions
                with torch.no_grad():
                    raw_cond = rssm_condition(rssm, obs, cond_actions, args.conditioning)
                cond = nets['cond_fuser'](raw_cond)
                noise_pred = nets['noise_pred_net'](noisy_actions, timesteps, global_cond=cond)
...
def main():
    ...
    parser.add_argument('--output', default='data/outputs/can_dp_rssm')
