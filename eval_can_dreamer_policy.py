...
from train_can_dreamer_policy import raw_condition
...
def policy_cond(rssm, nets, obs, actions, conditioning):
    with torch.no_grad():
        raw = raw_condition(rssm, obs, actions, conditioning)
    return nets['cond_fuser'](raw)
...
def eval_policy(args):
    ...
    cfg = payload['config']
    stats = payload['stats']
    rssm, _ = load_dreamer_rssm(args.rssm_checkpoint or cfg['rssm_checkpoint'], device)
    ...
    for ep in range(args.num_episodes):
        ...
        success_step = None
        step_idx = 0
        with tqdm(total=args.max_steps, desc=f'Dreamer eval episode {ep}', leave=False) as pbar:
            while step_idx < args.max_steps:
                ...
                with torch.no_grad():
                    ...
                    for k in scheduler.timesteps:
                        ...
                        pred = nets['noise_pred_net'](naction, k, global_cond=cond)
                        naction = scheduler.step(pred, k, naction).prev_sample
                action_pred = unnormalize_data(naction.detach().cpu().numpy()[0], stats['action'])
                for action in action_pred[cfg['obs_horizon'] - 1:cfg['obs_horizon'] - 1 + cfg['action_horizon']]:
        ...
        video_path = ''
        if args.save_videos < 0 or ep < args.save_videos:
...
def main():
    ...
    parser.add_argument('--output', default='data/eval_can_dp_dreamer_rssm')
